...
// NewPoisson cria uma nova distribuição de Poisson
func NewPoisson(lambda float64) (*Poisson, error) {
	...
	return &Poisson{
		lambda: lambda,
		dist:   distuv.Poisson{Lambda: lambda, Src: nil},
	}, nil
}

func (p *Poisson) Sample(rng *rand.Rand) float64 { // Atualizado para *math/rand/v2.Rand
	p.dist.Src = rng
	return p.dist.Rand()
}
