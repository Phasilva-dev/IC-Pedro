...
function enforce_minimum_gap(entry_time::Int32, exit_time::Int32, gap::Int32)
	if exit_time - entry_time < gap
...
function create_routine(profile::RoutineProfileWithOneEvent, rng::AbstractRNG, minimum_time_gap::Int32)
	wakeUp = generate_cyclic_time(rng, profile.wakeUpTime)
  sleep = generate_cyclic_time(rng, profile.sleepTime)

	leave_time = generate_cyclic_time(rng, profile.event.leaveTime)
	leave_time = enforce_minimum_gap(sleep,leave_time, minimum_time_gap)
...
function create_routine(profile::RoutineProfileWithEvents, rng::AbstractRNG, minimum_time_gap::Int32)
	wakeUp = generate_cyclic_time(rng, profile.wakeUpTime)
  sleep = generate_cyclic_time(rng, profile.sleepTime)

	events = Matrix{Int32}(undef, length(profile.leaveReturnTimes), 2)
...
function generate_cyclic_time(rng::AbstractRNG, dist::NormalDistParams)
		...
		Ndist = Normal(dist.mean, dist.std)
    result = trunc(Int, rand(rng, Ndist))  # Gera número NormalDistParams com RNG específico e arredonda
		result = result % DAY_IN_SECONDS
		if result < 0
			result += DAY_IN_SECONDS
		end # Ajusta para o intervalo [0, 86400)
    return Int32(result)  # Retorna como UInt32
end